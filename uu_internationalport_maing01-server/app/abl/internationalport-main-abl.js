...
const Errors = require("../api/errors/internationalport-main-error.js");
...
const logger = LoggerFactory.get("InternationalportMainAbl");

class InternationalportMainAbl {
  constructor() {
    this.validator = Validator.load();
    this.dao = DaoFactory.getDao("internationalportMain");
  }
  ...
  async init(uri, dtoIn, session) {
    ...
    // HDS 2
    const schemas = ["internationalportMain"];
    ...
    if (dtoIn.uuBtLocationUri) {
      ...
      const createAwscDtoIn = {
        name: "UuInternationalport",
        typeCode: "uu-internationalport-maing01",
        location: location,
    ...
    let uuInternationalPort;

    try {
      uuInternationalPort = await this.dao.create({ ...dtoIn, awid });
    } catch (e) {
      throw new Errors.Init.CreateuuInternationalPortFailed({ uuAppErrorMap }, { cause: e });
    }
    // HDS N+1
...
module.exports = new InternationalportMainAbl();
